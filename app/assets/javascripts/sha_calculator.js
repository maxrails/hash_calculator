...
self.addEventListener('message', function (event) {
    ...
    var uint8_array, message, block, message_length, output, block_size;
    ...
    if (block.end === block.file_size) {
        message_length = block.file_size * 8;
        block_size = block.end - block.start;

        // Padding
        message[(block_size * 8) >>> 5] |= 0x80 << (24 - message_length % 32);
        message[(((block_size * 8) + 64 >>> 9) << 4) + 15] = message_length;
